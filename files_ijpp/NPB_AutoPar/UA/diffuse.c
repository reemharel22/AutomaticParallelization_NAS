...
#pragma omp parallel for private (ie,i,j,k) reduction (+:rho1)
  for (ie = 0; ie <= nelt - 1; ie += 1) {
    
#pragma omp parallel for private (i,j,k) reduction (+:rho1)
    for (k = 0; k <= 4; k += 1) {
      
#pragma omp parallel for private (i,j) reduction (+:rho1)
      for (j = 0; j <= 4; j += 1) {
        
#pragma omp parallel for private (i) reduction (+:rho1)
        for (i = 0; i <= 4; i += 1) {
...
#pragma omp parallel for private (im) reduction (+:rho1)
...
#pragma omp parallel for private (ie,i,j,k) reduction (+:rho_aux)
      for (ie = 0; ie <= nelt - 1; ie += 1) {
        
#pragma omp parallel for private (i,j,k) reduction (+:rho_aux)
        for (k = 0; k <= 4; k += 1) {
          
#pragma omp parallel for private (i,j) reduction (+:rho_aux)
          for (j = 0; j <= 4; j += 1) {
            
#pragma omp parallel for private (i) reduction (+:rho_aux)
            for (i = 0; i <= 4; i += 1) {
...
#pragma omp parallel for private (im) reduction (+:rho_aux)
      for (im = 0; im <= nmor - 1; im += 1) {
...
#pragma omp parallel for private (ie,i,j,k) reduction (+:cona) firstprivate (nelt)
    for (ie = 0; ie <= nelt - 1; ie += 1) {
      
#pragma omp parallel for private (i,j,k) reduction (+:cona)
      for (k = 0; k <= 4; k += 1) {
        
#pragma omp parallel for private (i,j) reduction (+:cona)
        for (j = 0; j <= 4; j += 1) {
          
#pragma omp parallel for private (i) reduction (+:cona)
          for (i = 0; i <= 4; i += 1) {
...
#pragma omp parallel for private (im) reduction (+:cona)
    for (im = 0; im <= nmor - 1; im += 1) {
...
#pragma omp parallel for private (i,j,k,iz)
  for (iz = 0; iz <= 4; iz += 1) {
    for (k = 0; k <= 4; k += 1) {
...
#pragma omp parallel for private (i,j,iz)
    for (iz = 0; iz <= 4; iz += 1) {
...
#pragma omp parallel for private (i) firstprivate (sizei,rdtime)
      for (i = 0; i <= 4; i += 1) {
