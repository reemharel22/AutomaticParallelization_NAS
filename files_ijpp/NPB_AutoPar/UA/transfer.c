...
#pragma omp parallel for private (i,ig,il,ije1,ije2,col,j)
        for (ije1 = 0; ije1 <= nnje - 1; ije1 += 1) {
          for (ije2 = 0; ije2 <= nnje - 1; ije2 += 1) {
            
#pragma omp parallel for private (i,ig,il,col,j)
...
#pragma omp parallel for private (ig,il,i,j)
              for (i = 1; i <= 3; i += 1) {
...
#pragma omp parallel for private (ie) firstprivate (nmor)
  for (ie = 0; ie <= nmor - 1; ie += 1) {
    tmor[ie] = 0.0;
  }
  for (ie = 0; ie <= nelt - 1; ie += 1) {
    for (iface = 0; iface <= 5; iface += 1) {
...
#pragma omp parallel for private (shift,tmp,j,il,ije2,col,i)
        for (ije2 = 0; ije2 <= nnje - 1; ije2 += 1) {
          shift = ije2;
          
#pragma omp parallel for private (tmp,j,il,col,i) firstprivate (shift)
          for (col = 0; col <= 4; col += 1) {
...
#pragma omp parallel for private (il,i) reduction (+:tmp) firstprivate (j)
            for (i = 1; i <= 3; i += 1) {
              il = idel[ie][iface][i][col];
...
#pragma omp parallel for private (tmp,il,i,j)
            for (j = 2 - shift - 1; j <= 5 - shift - 1; j += 1) {
              tmp = 0.0;
              
#pragma omp parallel for private (il,i) reduction (+:tmp)
              for (i = 1; i <= 3; i += 1) {
                il = idel[ie][iface][i][col];
                tmp = tmp + qbnew[ije2][j][i - 1] * tx[il];
              }
...
#pragma omp parallel for private (i) reduction (+:tmp,tmp1)
              for (i = 1; i <= 3; i += 1) {
                tmp = tmp + qbnew[ije2][j][i - 1] * temp[ije1][col][i];
...
#pragma omp parallel for private (i,col,j)
  for (col = 0; col <= 4; col += 1) {
    i = 0;
...
#pragma omp parallel for private (j,col,i)
  for (col = 0; col <= 4; col += 1) {
    temp[0][col] = tx[0][col];
    j = 0;
    bottom[col] = 0.0;
...
#pragma omp parallel for private (col,j,i)
...
#pragma omp parallel for private (j) firstprivate (nmor)
  for (j = 0; j <= nmor - 1; j += 1) {
    tmort[j] = 0.0;
  }
  for (ie = 0; ie <= nelt - 1; ie += 1) {
    for (iface = 0; iface <= 5; iface += 1) {
