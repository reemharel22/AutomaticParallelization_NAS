...
#pragma omp parallel for private (k,i,j,q)
    for (k = 0; k <= 4; k += 1) {
      
#pragma omp parallel for private (i,j,q)
      for (j = 0; j <= 4; j += 1) {
        
#pragma omp parallel for private (i,q) firstprivate (rdtime,isize)
        for (i = 0; i <= 4; i += 1) {
          for (q = 0; q <= 4; q += 1) {
...
#pragma omp parallel for private (i,j)
  ...
  for (element_size = 0; element_size <= 7; element_size += 1) {
...
#pragma omp parallel for private (imor) firstprivate (nvertex)
  for (imor = 0; imor <= nvertex - 1; imor += 1) {
    ifpcmor[imor] = false;
  }
  
#pragma omp parallel for private (i,iel,iside)
  for (iel = 0; iel <= nelt - 1; iel += 1) {
    
#pragma omp parallel for private (i,iside)
...
#pragma omp parallel for private (ie,sizei,cornernumber,sface,sedge,iface,iedge,n,inemo,iiface,iiedge) reduction (+:tmortemp)
...
#pragma omp parallel for private (iface,iiface) reduction (+:sface)
...
#pragma omp parallel for private (iedge,iiedge) reduction (+:sedge) firstprivate (ie,cornernumber)
