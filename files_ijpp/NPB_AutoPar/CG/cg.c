...
//---------------------------------------------------------------------
  ...
  print_results("CG",Class,150000,0,0,75,t,mflops,"          floating point",verified,"3.3.1","23 Oct 2018","icc","$(CC)","-lm","-I../common","-g -Wall -O3 -qopenmp -mcmodel=medium","-O3 -qopenmp -mcmodel=medium","randdp");
//---------------------------------------------------------------------
// More timers
//---------------------------------------------------------------------
  if (timeron) {
...
#pragma omp parallel for private (suml,j,k)
    for (j = 0; j <= lastrow - firstrow + 1 - 1; j += 1) {
      suml = 0.0;
      
#pragma omp parallel for private (k) reduction (+:suml)
      for (k = rowstr[j]; k <= rowstr[j + 1] - 1; k += 1) {
...
#pragma omp parallel for private (suml,j,k) firstprivate (firstrow,lastrow)
  for (j = 0; j <= lastrow - firstrow + 1 - 1; j += 1) {
    suml = 0.0;
...
#pragma omp parallel for private (suml,j) reduction (+:sum) firstprivate (firstcol,lastcol)
  for (j = 0; j <= lastcol - firstcol + 1 - 1; j += 1) {
    suml = x[j] - r[j];
    sum = sum + suml * suml;
  }
...
//---------------------------------------------------------------------
  ...
  if (ihigh > n) 
    ihigh = n;
  for (iouter = 0; iouter <= ihigh - 1; iouter += 1) {
    nzv = 15;
...
//---------------------------------------------------------------------
  sparse(a,colidx,rowstr,n,nz,15,arow,acol,aelt,firstrow,lastrow,last_n,v,&iv[0],&iv[nz],1.0e-1,110.0);
}
//---------------------------------------------------------------------
...
#pragma omp parallel for private (i) reduction (+:nzrow)
    for (i = 0; i <= myid - 1; i += 1) {
      nzrow = nzrow + last_n[i];
    }
  }
  if (nzrow > 0) {
    
#pragma omp parallel for private (j) firstprivate (j2,nzrow)
    for (j = j1; j <= j2 - 1; j += 1) {
...
#pragma omp parallel for private (i) reduction (+:nzrow) firstprivate (myid)
    for (i = 0; i <= myid - 1; i += 1) {
      nzrow = nzrow + last_n[i];
    }
  }
...
#pragma omp parallel for private (j) firstprivate (ihigh,nzrow)
    for (j = ilow; j <= ihigh - 1; j += 1) {
