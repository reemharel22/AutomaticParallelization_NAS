...
void mortar()
{
	...
	#pragma cetus private(cb, cb1, edge_g, face2, i, ie, ie2, iel, ii, jface, jj, ne, ntemp, space) 
	#pragma loop name mortar#5 
	for (iel=0; iel<nelt; iel ++ )
	{
		front[iel]=(front[iel]-newc[iel]);
		count=(nvertex+front[iel]);
		#pragma cetus private(cb, cb1, edge_g, face2, i, ie, ie2, ii, jface, jj, ne, ntemp, space) 
		#pragma loop name mortar#5#0 
		for (i=0; i<6; i ++ )
		{
			cb1=cbc[iel][i];
			if (i<2)
			...
			if (fassign[iel][i])
			{
				...
				if ((cb==1)||(cb==0))
				{
					...
					#pragma loop name mortar#5#0#1 
					for (jj=1; jj<(5-1); jj ++ )
					{
						#pragma cetus private(ii) 
						#pragma loop name mortar#5#0#1#0 
						for (ii=1; ii<(5-1); ii ++ )
						{
				...
				else
				{
					if (cb==2)
					{
						if (idmo[iel][i][0][0][1][1]==( - 1))
						{
							...
							jface=jjface[i];
							#pragma cetus private(ii, jj) 
							#pragma loop name mortar#5#0#2 
							for (jj=1; jj<(5-1); jj ++ )
							{
								#pragma cetus private(ii) 
								#pragma loop name mortar#5#0#2#0 
								for (ii=1; ii<(5-1); ii ++ )
								{
									idmo[iel][i][0][0][jj][ii]=count;
	...
	#pragma loop name mortar#6 
	for (iel=0; iel<nelt; iel ++ )
	{
		#pragma cetus private(cb, i, ii, iii, jface, jj, jjj, ntemp) 
		#pragma loop name mortar#6#0 
		for (i=0; i<6; i ++ )
		{
			cb=cbc[iel][i];
			if (cb==3)
			...
			if (cb==3)
			{
				...
				#pragma loop name mortar#6#0#0 
				for (iii=0; iii<2; iii ++ )
				{
					#pragma cetus private(ii, jj, jjj, ntemp) 
					#pragma loop name mortar#6#0#0#0 
					for (jjj=0; jjj<2; jjj ++ )
					{
						ntemp=sje[iel][i][jjj][iii];
						#pragma cetus private(ii, jj) 
						#pragma loop name mortar#6#0#0#0#0 
						#pragma cetus parallel 
						/*
						...
						*/
						...
						{
							#pragma cetus private(ii) 
							#pragma loop name mortar#6#0#0#0#0#0 
							for (ii=0; ii<5; ii ++ )
							{
...
static void mor_edge(int ie, int face, int iel, int mor_v[3])
{
	...
	{
		j=0;
		#pragma cetus private(nn) 
		#pragma loop name mor_edge#0 
		#pragma cetus parallel 
		/*
		Disabled due to low profitability: #pragma omp parallel for private(nn)
		*/
		for (nn=1; nn<(5-1); nn ++ )
		{
