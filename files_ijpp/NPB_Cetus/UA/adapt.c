...
void adaptation(logical * ifmortar, int step)
{
	...
	#pragma omp parallel for if((10000<(1L+(3L*nelt)))) private(iel)
...
static void do_coarsen(logical * if_coarsen, int * icoarsen, int neltold)
{
	...
	#pragma cetus parallel 
	#pragma omp parallel for if((10000<(1L+(3L*neltold)))) private(iel)
	...
	for (miel=0; miel<nelt; miel ++ )
	{
		...
		if (ich[iel]==2)
		{
			...
			if ((( ! (ic&(1<<0)))&&( ! (ic&(1<<1))))&&( ! (ic&(1<<2))))
			{
				...
				#pragma loop name do_coarsen#2#0 
				for (i=0; i<8; i ++ )
				{
				...
				if (test1)
				{
					test2=true;
					#pragma cetus private(i) 
					#pragma loop name do_coarsen#2#1 
					for (i=0; i<8; i ++ )
					{
						if (ich[ntp[i]]!=2)
						{
					...
					if (test2)
					{
						test3=true;
						#pragma cetus private(i) 
						#pragma loop name do_coarsen#2#2 
						for (i=0; i<8; i ++ )
						{
							if ( ! icheck(ntp[i], i))
							{
				...
				if (test)
				{
					ifcoa[miel]=true;
					#pragma cetus private(i) 
					#pragma loop name do_coarsen#2#3 
					for (i=0; i<8; i ++ )
					{
						ifcoa_id[ntp[i]]=true;
					}
					front[miel]=1;
					#pragma cetus private(i) 
					#pragma loop name do_coarsen#2#4 
					for (i=0; i<7; i ++ )
					{
						skip[ntp[i+1]]=true;
					}
...
static void do_refine(logical * ifmortar, int * irefine)
{
	...
	#pragma cetus parallel 
	#pragma omp parallel for if((10000<(1L+(8L*nelt)))) private(miel)
	for (miel=0; miel<nelt; miel ++ )
	{
	...
	#pragma cetus private(cb, cbctemp, facedir, i, iel, ii, index, j, jface, jj, k, le, miel, mielnew, n1, n2, ndir, ne, ntemp, sjetemp, treetemp, xctemp, xhalf, xleft, xright, yctemp, yhalf, yleft, yright, zctemp, zhalf, zleft, zright) 
	...
	for (index=0; index<num_refine; index ++ )
	{
		...
		*/
		...
		#pragma cetus parallel 
		/*
		Disabled due to low profitability: #pragma omp parallel for private(i, ii, jj)
		*/
		for (i=0; i<6; i ++ )
		{
			cbctemp[i]=cbc[iel][i];
			#pragma cetus private(ii, jj) 
			#pragma loop name do_refine#3#1#0 
			for (jj=0; jj<2; jj ++ )
			{
				#pragma cetus private(ii) 
				#pragma loop name do_refine#3#1#0#0 
				for (ii=0; ii<2; ii ++ )
				{
		...
		*/
		...
		{
			#pragma cetus private(i) 
			#pragma loop name do_refine#3#4#0 
			for (i=0; i<7; i+=2)
			{
				xc[nelt+j][i]=xhalf;
		...
		*/
		...
		{
			#pragma cetus private(i) 
			#pragma loop name do_refine#3#5#0 
			for (i=0; i<7; i+=2)
			{
				xc[nelt+j][i]=xleft;
				xc[nelt+j][i+1]=xhalf;
		...
		*/
		...
		{
			#pragma cetus private(i) 
			#pragma loop name do_refine#3#9#0 
			for (i=0; i<2; i ++ )
			{
				yc[nelt+j][i]=yhalf;
				yc[(nelt+j)+4][i]=yhalf;
			...
			#pragma cetus private(i) 
			#pragma loop name do_refine#3#9#1 
			for (i=2; i<4; i ++ )
			{
				yc[nelt+j][i]=yright;
		...
		*/
		...
		{
			#pragma cetus private(i) 
			#pragma loop name do_refine#3#12#0 
			for (i=0; i<4; i ++ )
			{
				zc[nelt+j][i]=zleft;
				zc[nelt+j][i+4]=zhalf;
		...
		*/
		...
		#pragma cetus private(i) 
		#pragma loop name do_refine#3#14 
		#pragma cetus parallel 
		/*
		Disabled due to low profitability: #pragma omp parallel for private(i)
		*/
		...
		for (ndir=0; ndir<3; ndir ++ )
		{
			...
			for (facedir=0; facedir<=1; facedir ++ )
			{
				...
				#pragma loop name do_refine#3#15#0#0 
				#pragma cetus parallel 
				/*
				...
				*/
				...
				/* face of the parent element */
				#pragma cetus private(k) 
				#pragma loop name do_refine#3#15#0#1 
				for (k=0; k<4; k ++ )
				{
					cbc[le[k]][i]=2;
					sje[le[k]][i][0][0]=ne[k];
				...
				if (cb==2)
				{
					...
					if (ich[ntemp]!=4)
					{
						...
						#pragma loop name do_refine#3#15#0#2 
						for (k=0; k<4; k ++ )
						{
							cbc[ne[k]][i]=1;
					...
					else
					{
						...
						#pragma loop name do_refine#3#15#0#3 
						for (k=0; k<4; k ++ )
						{
							cbc[ne[k]][i]=2;
				...
				else
				{
					...
					{
						#pragma cetus private(k) 
						#pragma loop name do_refine#3#15#0#4 
						for (k=0; k<4; k ++ )
						{
							cbc[ne[k]][i]=2;
							if (k==0)
					...
					else
					{
						...
						{
							#pragma cetus private(k) 
							#pragma loop name do_refine#3#15#0#5 
							#pragma cetus parallel 
							/*
							Disabled due to low profitability: #pragma omp parallel for private(k)
							*/
...
static void remap(double y[5][5][5], double y1[7][5][5][5], double x[5][5][5])
{
	...
	*/
	for (i=0; i<5; i ++ )
	{
		...
		/* #pragma cetus reduction(+: yone[0][i][jj][ii], yone[1][i][jj][ii])  */
		for (kk=0; kk<5; kk ++ )
		{
			#pragma cetus private(ii, jj) 
		...
		/* #pragma cetus reduction(+: ytwo[0][jj][i][ii], ytwo[1][jj][i][ii], ytwo[2][jj][i][ii], ytwo[3][jj][i][ii])  */
		for (kk=0; kk<5; kk ++ )
		{
			#pragma cetus private(ii, jj) 
	...
	*/
	for (iz=0; iz<5; iz ++ )
	{
		...
		/* #pragma cetus reduction(+: y1[0][jj][iz][ii], y1[1][jj][iz][ii], y1[2][jj][iz][ii], y1[3][jj][iz][ii], y1[4][jj][iz][ii], y1[5][jj][iz][ii], y1[6][jj][iz][ii], y[jj][iz][ii])  */
		for (kk=0; kk<5; kk ++ )
		{
			#pragma cetus private(ii, jj) 
	...
	*/
	for (iy=0; iy<5; iy ++ )
	{
		...
		for (ix=0; ix<5; ix ++ )
		{
			...
			/* #pragma cetus reduction(+: y[1][iy][ix])  */
			for (ip=0; ip<5; ip ++ )
			{
			...
			/* #pragma cetus reduction(+: y[3][iy][ix])  */
			for (ip=0; ip<5; ip ++ )
			{
	...
	*/
	for (iz=0; iz<5; iz ++ )
	{
		...
		for (ix=0; ix<5; ix ++ )
		{
			...
			/* #pragma cetus reduction(+: y[iz][1][ix])  */
			for (ip=0; ip<5; ip ++ )
			{
			...
			/* #pragma cetus reduction(+: y[iz][3][ix])  */
			for (ip=0; ip<5; ip ++ )
			{
	...
	*/
	for (iz=0; iz<5; iz ++ )
	{
		...
		for (iy=0; iy<5; iy ++ )
		{
			...
			/* #pragma cetus reduction(+: y[iz][iy][1])  */
			for (ip=0; ip<5; ip ++ )
			{
			...
			/* #pragma cetus reduction(+: y[iz][iy][3])  */
			for (ip=0; ip<5; ip ++ )
			{
