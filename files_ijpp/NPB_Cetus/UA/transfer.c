				...
				for (ije1=0; ije1<nnje; ije1 ++ )
				{
					...
					for (ije2=0; ije2<nnje; ije2 ++ )
					{
						...
						for (j=0; j<5; j ++ )
						{
							...
							#pragma cetus parallel 
							/*
							Disabled due to low profitability: #pragma omp parallel for private(i) reduction(+: tmp, tmp1)
							*/
							for (i=1; i<(5-1); i ++ )
							{
								tmp=(tmp+(qbnew[ije2][j][i-1]*temp[ije1][col][i]));
				...
				if (idmo[ie][iface][0][0][0][5-1]!=( - 1))
				{
					...
					for (ije=0; ije<2; ije ++ )
					{
						...
						for (j=0; j<5; j ++ )
						{
							...
							#pragma cetus parallel 
							/*
							Disabled due to low profitability: #pragma omp parallel for private(i, il) reduction(+: tmp)
							*/
							for (i=1; i<(5-1); i ++ )
							{
								il=idel[ie][iface][0][i];
