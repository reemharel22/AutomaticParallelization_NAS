...
void geom1()
{
	...
	*/
	...
	for (iel=0; iel<33500; iel ++ )
	{
		ntemp=(((5*5)*5)*iel);
		#pragma cetus private(i, j) 
		#pragma loop name geom1#2#0 
		for (j=0; j<5; j ++ )
		{
			#pragma cetus private(i) 
			#pragma loop name geom1#2#0#0 
			for (i=0; i<5; i ++ )
			{
	...
	*/
	for (i=0; i<5; i ++ )
	{
		...
		for (j=0; j<5; j ++ )
		{
			...
			/* #pragma cetus reduction(+: wdtdr[j][i])  */
			for (ip=0; ip<5; ip ++ )
			{
...
void prepwork()
{
	...
	#pragma cetus parallel 
	#pragma omp parallel for if((10000<(1L+(3L*nmor)))) private(iel)
	for (iel=0; iel<nmor; iel ++ )
	{
		tmmor[iel]=1.0;
	}
	#pragma cetus private(cb, i, iel, iface, j) 
	#pragma loop name prepwork#3 
	for (iel=0; iel<nelt; iel ++ )
	{
		#pragma cetus private(cb, i, iface, j) 
		#pragma loop name prepwork#3#0 
		for (iface=0; iface<6; iface ++ )
		{
			cb=cbc[iel][iface];
			if (cb==0)
			{
				#pragma cetus private(i, j) 
				#pragma loop name prepwork#3#0#0 
				for (j=1; j<(5-1); j ++ )
				{
					#pragma cetus private(i) 
					#pragma loop name prepwork#3#0#0#0 
					for (i=1; i<(5-1); i ++ )
					{
				...
				j=0;
				#pragma cetus private(i) 
				#pragma loop name prepwork#3#0#1 
				for (i=0; i<(5-1); i ++ )
				{
					tmmor[idmo[iel][iface][0][0][j][i]]=0.0;
				}
				...
				else
				{
					...
					#pragma cetus private(i) 
					#pragma loop name prepwork#3#0#2 
					for (i=0; i<5; i ++ )
					{
				...
				if (idmo[iel][iface][1][0][1][5-1]==( - 1))
				{
					...
					#pragma loop name prepwork#3#0#3 
					for (j=1; j<(5-1); j ++ )
					{
						tmmor[idmo[iel][iface][0][0][j][i]]=0.0;
					}
				...
				{
					#pragma cetus private(j) 
					#pragma loop name prepwork#3#0#4 
					for (j=1; j<5; j ++ )
					{
						tmmor[idmo[iel][iface][1][0][j][i]]=0.0;
					}
					#pragma cetus private(j) 
					#pragma loop name prepwork#3#0#5 
					for (j=0; j<5; j ++ )
					{
				...
				{
					#pragma cetus private(i) 
					#pragma loop name prepwork#3#0#6 
					for (i=1; i<(5-1); i ++ )
					{
						tmmor[idmo[iel][iface][0][0][j][i]]=0.0;
					}
				...
				{
					#pragma cetus private(i) 
					#pragma loop name prepwork#3#0#7 
					for (i=1; i<5; i ++ )
					{
						tmmor[idmo[iel][iface][0][1][j][i]]=0.0;
					}
					#pragma cetus private(i) 
					#pragma loop name prepwork#3#0#8 
					for (i=0; i<(5-1); i ++ )
					{
						tmmor[idmo[iel][iface][1][1][j][i]]=0.0;
					}
				...
				i=0;
				#pragma cetus private(j) 
				#pragma loop name prepwork#3#0#9 
				for (j=1; j<(5-1); j ++ )
				{
					tmmor[idmo[iel][iface][0][0][j][i]]=0.0;
				}
				if (idmo[iel][iface][0][0][5-1][0]!=( - 1))
				{
					...
					#pragma cetus private(j) 
					#pragma loop name prepwork#3#0#10 
					for (j=0; j<(5-1); j ++ )
					{
						tmmor[idmo[iel][iface][0][1][j][i]]=0.0;
					}
